...
try {
    ...
    logMessage("Comando FFmpeg (HLS): $command1");
    ...
    logMessage("Comando FFmpeg (Miniatura): $command2");
...
function executeFFmpegCommand($command, $maxAttempts = 3) {
    for ($attempt = 1; $attempt <= $maxAttempts; $attempt++) {
        logMessage("Intento $attempt de $maxAttempts - Ejecutando comando FFmpeg: $command");
        $output = shell_exec($command . " 2>&1");
        if ($output !== null && strpos($output, 'error') === false) {
            return $output;
        }
        logMessage("Intento $attempt fallido. Salida: " . $output);
